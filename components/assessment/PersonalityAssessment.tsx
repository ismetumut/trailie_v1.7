...
// 20 placeholder questions for MVP
const questions = [
  ...
  { key: "empathyQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "organizationQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "decisionMakingQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "motivationQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "confidenceQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "patienceQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "responsibilityQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "curiosityQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "planningQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "assertivenessQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "collaborationQ", options: ["always", "often", "sometimes", "rarely"] },
  { key: "focusQ", options: ["always", "often", "sometimes", "rarely"] },
]

export default function PersonalityAssessment({ language, onComplete }: { language: Language, onComplete: (answers: string[]) => void }) {
  ...
  async function savePersonalityResults(answers: string[]) {
  ...
  function handleNext() {
    if (step < questions.length - 1) setStep(step + 1)
      ...
      savePersonalityResults(answers as string[])
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-white rounded-2xl shadow-xl mt-4 font-sans">
      <div className="mb-2 text-xs text-primary font-semibold tracking-wide">{t('personalityInventory')} ({step+1}/{questions.length})</div>
